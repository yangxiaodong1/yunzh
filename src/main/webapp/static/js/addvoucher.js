...
function voucherSummary(){
	...
	$("body").on("click","td.col-summary",function(){ 
		...
		if($(this).find("input").hasClass("select")){
			$(this).find("input").removeClass("select");
...
$(this).parents("tr").find(".cellOn").find(".cell-val").find("p").eq(0).data("amountdc");
...
function voucherfunction(){
	...
	$("body").on("click",".a-add",function(){
		$(this).parent().parent().parent().after(voucherTr);
	})
	//点击删除
...
function templateFunction(){
	...
	//模板操作-模板导入
	$("body").on("click",".sp-import",function(){
		$(".mask").show();
		$("#tab").show();
	...
	$("body").on("click",".usetemplate",function(e){
		$("#tab").hide();
		$(".voucher-tab .tb").removeClass("voucher-current").hide();
		//$(".voucher-current").removeClass("voucher-current").hide();
		
		$(".voucher-tab").append($("#template-voucher").html());
		$(".voucher-tab .tb:last").addClass("voucher-current add-voucher-account");
		//图片展览隐藏
		$(".pics").hide();
		getRememberWord();
		//获取模板信息  更新表格内容
		$.ajax({
		     ...
		     success: function(result){
		    	...
		    	for( i in data){
		    		...
		    		//陈明加的
		    		var end=getLastAccountEndBalance(data[i].accountId).endbalance;
		    		if(data[i].dc == "-1"){
		    			if(end!='0'){
		    				end=-1*end;
		    	...
		    	$(".voucher-tab").find(".voucher-current").find(".tbody-voucher").html(str);
		    	$(".voucher-tab").find(".voucher-current").show();
		     	countVoucherTotal();
		     	$(".mask").hide();
...
function saveVoucher(){
	...
	$(".voucher-current").find(".entry-item").each(function(i){
		...
		if(exp.length > 0){
			...
			noinputaccountName = $(this).find(".subject-val").find("p").eq(0);
			//var accountaaa = $(this).find(".subject-val").text();
			if($(noinputaccountName).text().trim().length < 1 && !flagAccount){
				flagAccount = true;
				noAccountVar = noinputaccountName;
			}
			var saveFlag = true;
			if($(noinputaccountName).text().trim().length < 1){
				saveFlag = false;
			}
			voucherExp.accountName =  $(noinputaccountName).text();
			voucherExp.accountId =  $(this).find(".subject-val").find("p").eq(0).data("amountid");
	...
	if(flag){
		//messagePop("第1条分录摘要不能为空！");
		$(".span-tip").text("第1条分录摘要不能为空！").show();
		msgFadeOut();
		return false;
	}
	
	if(flagAccount){ //科目未填 未填处于点击状态
		$(noinputaccountName).click();
		$(".span-tip").text("科目不能为空").show();
		msgFadeOut();
		return false;
	}
	...
	if(validTr < 2){
		//messagePop("请录入至少二条有效分录！");
		$(".span-tip").text("请录入至少二条有效分录！").show();
		msgFadeOut();
		return false;
	}
...
function getRememberWord(){
	$.ajax({
	     ...
	     success: function(data){
	    	 $(".voucher-current").find(".ui-spinbox").val(PrefixInteger(data+1, 3));
	     }
	});
}
...
function getLastAccountEndBalance(accountid){
	//var balance
	/*if($(accountid).text().trim().length < 1){
		return false;
	}*/
	$.ajax({
	     type: 'POST',
	...
	$.ajax({
	    ...
	    /*	for(var i in data){
	    	...
	    	$(data).each(function(e){
	    		str += "<option value='"+this.id+"'  data-ifamortize='"+this.ifAmortize+"'   data-level='"+this.level+"'>" +this.accuntId+"&nbsp;"+this.parentName+this.accountName+"</option>";
	    	})
