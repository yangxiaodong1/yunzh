			...
			<div class="t_pane_a1 radius_5">
				<div class="user_pane_r2">
					...
					<ul class="nav nav-tabs" role="tablist">
						...
						<li  class="active"><a href="${ctx}/workterrace/wages/">工资</a></li>
							...
							<div class="pane_gzt">
								<div class="search_form4">
										...
										<span class="span_year left">
											<form:form id="searchForm" modelAttribute="tCustomer" action="${ctx}/workterrace/wages/list" method="post" >
					...
					<input type="button" class="bg_p6 btn_sea2 left mar-rig3" onclick="down()" value="右"/>
								...
								<div class="fixed_table">
									<table class="table_jz2" id='myTable01' collspacing='0' cellpadding='0' border='1' width='100%'>
								      ...
								      <tbody>
								<c:forEach items="${listWagesBySupervisor}" var="tCustomer" varStatus="status">
								       <tr>
							...
							<div class=" bg_p11 line_h30 font_cc7 font-14 radius_bl5 radius_br5">
								<div class="pane_gzt">
									<span class="left"><span id="bymonth"></span>月无数据&nbsp;<span id="doWillCount">${tc.doWillCount }</span>&nbsp;&nbsp;&nbsp;有数据&nbsp;<span id="doneCount">${tc.doneCount }</span></span>
									<span class="right">
										<a href="${ctx}/workterrace/wages/list" class="left mar-rig20"><span class="btn btn-default btn_n4 bg_p6 btn_i8 pad_10"></span>重新统计</a>
...
$(function(){
	...
	$(".fixed_table thead th").on("click",function() {
		...
		$.post("${ctx}/workterrace/wages/wagesCount",{byYearMonth:yearmonth},function(result){
			$("#bymonth").text(month);
	...
	$("#bymonth").text(month+"");
	 ...
	 $("#tab_f1 table .btn_sta03").on("click",function() {
		 //alert($(this).parent().parent().html());
			//当前客户id
			var customerId=$(this).parent().parent().find("input:last").val();
			//当前账期
			var salaryPeriod= $(this).parent().parent().find("td").index($(this).parent())-1;
			if(salaryPeriod<10){
				salaryPeriod="0"+salaryPeriod;
			}
			salaryPeriod=$("#byYear").val()+salaryPeriod;
			var isok='0';
			//alert(customerId+"   "+salaryPeriod+"   "+$(this).parent().parent().parent().find("#currentperiod").val());
			
			if(salaryPeriod==$(this).parent().parent().find("#currentperiod").val()){
				isok='1';
			}
			window.open("${ctx}/workterrace/tCustomerSalary/list?salaryPeriod="+salaryPeriod+"&customerId="+customerId+"&isok="+isok);
			
		})
		
		$("#tab_f1 table .btn_sta01,.going").on("click",function() {
			// alert($(this).parent().parent().html());
		//当前客户id
		var customerId=$(this).parent().parent().find("input:last").val();
		//当前账期
		...
		if(salaryPeriod==$(this).parent().parent().find("#currentperiod").val()){
			...
			window.open("${ctx}/workterrace/tCustomerSalary/list?salaryPeriod="+salaryPeriod+"&customerId="+customerId+"&isok=1");
		}
		});
