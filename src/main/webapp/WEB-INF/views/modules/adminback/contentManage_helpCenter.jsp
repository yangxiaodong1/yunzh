...
<div class="pad_15_10">
	<div class="panel panel-default mar_b0 radius0 border_none">
		<div class="panel-heading bg_col_2 panel-head-pub">
			<div class=" pad_t_3">
				<span class="glyphicon glyphicon_a8 ico_pub bg_pub mar-top2" aria-hidden="true"></span><span class="font_cc8 mar-rig10 font-14">帮助中心</span>
				<span class=" add btn btn-default btn_w_a2 btn_bg_6 mar-rig10">添加</span>
			</div>
		...
		<table class="table table-center table_hover2 font_cc9">
			...
			<tbody>
				...
				<c:forEach items="${thList}" var="tl">
				<tr>
					<td class="text-left">${tl.title}</td>
					<td>${tl.createDateString}</td>
					<td>${tl.updateDateString}</td>
					<td>${tl.updateByStrinig}</td>
					<td class="font_cc4"><a href="${pageContext.request.contextPath}/a/helpcenter/thelpcenter/addHelp2?id=${tl.id}" class="mar-lr5">编辑</a><a href="#" class="mar-lr5 dialog_del" id="${tl.id}">删除</a></td>
				</tr>
				</c:forEach>		
...
$(function(){
	...
	$( "#dialog_del" ).dialog({
       ...
       $( ".dialog_del" ).click(function(){
		...
		//alert(id);
		var url='${pageContext.request.contextPath}/a/helpcenter/thelpcenter/deleteHelp?id='+id;
	...
	$(".add").on("click",function(){
		//alert(1);
		urls='${ctx}/helpcenter/thelpcenter/addHelp';
