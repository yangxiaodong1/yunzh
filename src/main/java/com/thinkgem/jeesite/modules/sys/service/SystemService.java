...
public class SystemService extends BaseService implements InitializingBean {
	...
	public int validatorLoginName(User user){
		return userDao.validatorLoginName(user);
	}
	/**
	 * 根据登录名获取用户
	 * @param loginName
	...
	public Page<User> findListNew(Page<User> page, User user) {
		// 设置分页参数
		user.setPage(page);
		// 执行分页查询
		page.setList(userDao.findListNew(user));
		return page;
	}
	public List<User> findListNew(User user) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		return userDao.findListNew(user);
	}
	/*
	 * 获取公司管理员
	 * */
	public List<User> findCompanyAdmin(User user) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		return userDao.findCompanyAdmin(user);
	}
	...
	public List<User> selectuserzNodes(User user){
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		//user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(), "o", "a"));
		List<User> list = userDao.selectuserzNodes(user);
		return list;
	}
	...
	public List<User> allUserList(){
		List<User> list=userDao.allUserList();
		return list;
	}
	/**
	...
	public List<User> findByOfficeId(String officeId) {
		...
		//if (list == null){
			User user = new User();
			user.setOfficeid("1");
			List<User> list = userDao.findByOfficeId(user);
			//CacheUtils.put(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + officeId, list);
		//}
		return list;
	}
	...
	public List<User> finddj(){
		return userDao.finddj();
	}
...
//			systemRealm.clearAllCachedAuthorizationInfo();
	...
	//保存权限
	@Transactional(readOnly = false)
	public void saveUserPower(User user) {
		
		if (StringUtils.isNotBlank(user.getId())){
			// 更新用户与权限关联
			userDao.deleteUserPower(user);
			//if (user.getPowerList() != null && user.getPowerList().size() > 0){
			if (user.getPowerIdList() != null && user.getPowerIdList().size() > 0){
				userDao.insertUserPower(user);
			}else{
				//throw new ServiceException(user.getLoginName() + "没有设置权限！");
			}
			// 将当前用户同步到Activiti
			saveActivitiUser(user);
			// 清除用户缓存
...
//		// 清除权限缓存
	...
	//更改用户的状态
		...
		public void updateuserStatus(User user) {
		 /*
			List<User> userList=userDao.findsoncount(user);
			for (User u : userList) {
				if (u.getOffice() != null && u.getOffice().getId() != null){
					CacheUtils.remove(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + u.getOffice().getId());
				}
			}*/
			User oldUser = userDao.get(user.getId());
			if (oldUser.getOffice() != null && oldUser.getOffice().getId() != null){
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List<User> findUserByCompanyid(User user){
		return userDao.findUserByCompanyid(user);
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)//yang
	public void updatePasswordByIdnew(User user){
		userDao.updatePasswordByIdnew(user);
		UserUtils.clearCache(user);
	}
	@Transactional(readOnly = false)
	...
	public User findByidnew(String id){
		return userDao.findByidnew(id);
	}
	/**
	 * yang
	 * @param id
	...
	public User findByidandaccount(User user){
		return userDao.findByidandaccount(user);
	}
	//获取子账号个数yang
	public List<User> findsoncount(User user){
		return userDao.findsoncount(user);
	}
	public List<User> findcompcount(User user){
		return userDao.findcompcount(user);
	}
	/**
	...
	public List<Role> findAllListByJZ(){
		return roleDao.findAllListByJZ();
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public static boolean printKeyLoadMessage(){
		...
		sb.append("\r\n======================================================================\r\n");
		sb.append("\r\n    欢迎使用 "+Global.getConfig("productName")+"  - Powered By 汇智容大 \r\n");
		sb.append("\r\n======================================================================\r\n");
	...
	public String inputbasedata(Map<String, Object> map){
		return userDao.inputbasedata(map);
	}
	...
	public String againinputbasedata(Map<String, Object> map){
		return userDao.againinputbasedata(map);
	}
	...
	public void insertchargeCompanyId(User user){
		if (user.getIsNewRecord()){
			user.preInsert();
			//userDao.insert(user);
			userDao.insertchargeCompanyId(user);
		}else{
			//user.preUpdate();
			//userDao.update(user);
		}
	...
	public List<User> findListNewLeave(User user) {
	 ...
	 return userDao.findListNewLeave(user);
	}

	public List<User> findUserListBySearch(String search, String companyId) {
		// TODO Auto-generated method stub
		return userDao.findUserListBySearch(search,companyId);
	}
